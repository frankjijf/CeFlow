...
load("C:\\Darren\\ziqing\\Merkle\\Model\\CE\\RCE\\data\\test_data.RData")

inlib <- "C:\\Darren\\ziqing\\Merkle\\Model\\CE\\RCE\\output\\continuous"
setwd(inlib)

View(head(test_data))
range(test_data$dv_revenue_in_attr)
# 0.0 103466.1
mean(test_data$dv_revenue_in_attr)
# 124.5409
...
mean(dat0[dat0$lrnval=='V',"dv_revenue_in_attr"])
# 121.3081
mean(dat0[dat0$lrnval=='L',"dv_revenue_in_attr"])
# 118.7114

## Specified Parameters
lab.targ<-'dv_revenue_in_attr'   ## Dependent variable
...
dat2 <- dat2[,names(dat2)[pmax(grepl("name",names(dat2),ignore.case=T),
                               ...
                               ,grepl("dv_any_purchase_flag_in_attr",names(dat2),ignore.case=T)
...
## Save model sample
save(dat2, file = "model_sample.RData")
