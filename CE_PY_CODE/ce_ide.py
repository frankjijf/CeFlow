...
# 侧边栏导航 标签
step_labels = ["1. Load The Dataset", "2. Variable Lists", "3. CE1 Sampling", "4. CE2 EDA & Var Recode", "5. CE3 Var Reduce"]
    ...
    if sampled is None:
        ...
        with left:
            with st.form("ce2_form", border=False):
                ...
                with st.expander("Options are ER for Equal Response method", expanded=False):
                    ...
                    missrate = st.number_input(label_missrate, value=0.75, key="missrate")

                    label_concrate = '''**CONCRATE**  
                    :blue-background[Maximum amount of file that can be in a single value of an independent variable]'''
                    concrate = st.number_input(label_concrate, value=0.9, key="concrate")
                    ...
                    minbinnc = st.number_input(label_minbinnc, value=500, key="minbinnc")

                    label_minbinnp = '''**MINBINNP**  
                    :blue-background[Minimum percent of file in a bin to be usable. Set to 0 to use minbincnt only]'''
                    minbinnp = st.number_input(label_minbinnp, value=0.05, key="minbinnp")
                    ...
                    nom_method = st.text_input(label_nom_method, "INDEX", key="nom_method")
                    ...
                    min_size = st.number_input(label_min_size, value=500, key="min_size")

                    label_num_category = '''**NUM_CATEGORY**  
                    :blue-background[Maximum number of categories for profiling variables]'''
                    num_category = st.number_input(label_num_category, value=10, key="num_category")
                    ...
                    impmethodC = st.text_input(label_impmethodC, "median", key="impmethodC")

                    label_stdmethodC = '''**STDMETHODC**  
                    :blue-background[Standardization options: any method allowed in proc stdize or NO to skip]'''
                    stdmethodC = st.text_input(label_stdmethodC, "STD", key="stdmethodC")
                    ...
                    transformationC = st.pills(label_transformationC, ["Y", "N"], default="Y", key="transformationC")

                    label_impmethodO = '''**IMPMETHODO**  
                    :blue-background[What method to use for missing imputation for ordinal variables?]'''
                    impmethodO = st.text_input(label_impmethodO, "mean", key="impmethodO")
        ...
        with right:
            if "recoded_df" in state:
                ...
                st.button("下一步: CE3 Var Reduce", on_click=go_to, args=(5,))
            else:
    ...
    with left:
        with st.form("ce3_form", border=False):
            ...
            with st.expander("Optional Macro Variables: These all have defaults that can be used", expanded=False):
                ...
                :blue-background[Request profiling report on all variables (Y/N)]'''
                samplesize = st.number_input(label_samplesize, value=50000, key="samplesize")

                label_redu_weight = '''**REDU_WEIGHT**  
                :blue-background[Use weights in variables reduction? (Y/N)]'''
                redu_weight = st.pills(label_redu_weight, ["Y", "N"], default="N", key="redu_weight")
                ...
                univ_reg_flg = st.pills(label_univ_reg_flg, ["Y", "N"], default="Y", key="univ_reg_flg")
                ...
                correlation_flg = st.pills(label_correlation_flg, ["Y", "N"], default="Y", key="correlation_flg")
                ...
                principal_flg = st.pills(label_principal_flg, ["Y", "N"], default="Y", key="principal_flg")
                label_nprin = '''**NPRIN**  
                ...
                cluster_flg = st.pills(label_cluster_flg, ["Y", "N"], default="Y", key="cluster_flg")
                label_maxc = '''**MAXC**  
                ...
                regression_flg = st.pills(label_regression_flg, ["Y", "N"], default="Y", key="regression_flg")
                ...
                st.markdown("##### :rainbow[**Logistic regression option - only applicable if binary dependent variable**]")
                label_logistic_flg = '''**LOGISTIC_FLG**  
                :blue-background[Use logistic regression to choose variables? (Y/N)]'''
                logistic_flg = st.pills(label_logistic_flg, ["Y", "N"], default="Y", key="logistic_flg")
                ...
                information_flg = st.pills(label_information_flg, ["Y", "N"], default="Y", key="information_flg")
                label_decile = '''**DECILE**  
                ...
                st.markdown("##### :rainbow[**Maximum correlation between independent variables option**]")
                label_ind_correlation_flg = '''**IND_CORRELATION_FLG**  
                :blue-background[Exclude variables with high correlation to others? (Y/N)]'''
                ind_correlation_flg = st.pills(label_ind_correlation_flg, ["Y", "N"], default="Y", key="ind_correlation_flg")
                ...
                st.markdown("##### :rainbow[**Maximum correlation to dependent variable option**]")
                label_ind_dv_corr_flg = '''**IND_DV_CORR_FLG**  
                :blue-background[Exclude variables with high correlation to dependent variable? (Y/N)]'''
                ind_dv_corr_flg = st.pills(label_ind_dv_corr_flg, ["Y", "N"], default="Y", key="ind_dv_corr_flg")
                label_max_dv_corr = '''**MAX_DV_CORR**  
                :blue-background[Maximum correlation allowed to dependent variable]'''
                max_dv_corr = st.number_input(label_max_dv_corr, value=0.7, key="max_dv_corr")

            submitted3 = st.form_submit_button("运行 CE3")
        if submitted3:
            os.makedirs(out_dir, exist_ok=True)
            log_dir = os.path.join(out_dir, "log")
            os.makedirs(log_dir, exist_ok=True)
