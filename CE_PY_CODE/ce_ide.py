...
# -------- Step 2: 变量确认与配置 --------
    ...
    if df is None:
        ...
        state.bin_vars = st.multiselect(
            "二元变量", df.columns.tolist(), default=default_bin
        )
        state.nom_vars = st.multiselect(
            "名义变量", df.columns.tolist(), default=default_nom
        )
        state.ord_vars = st.multiselect(
            "有序变量", df.columns.tolist(), default=default_ord
        )
        state.cont_vars = st.multiselect(
            "连续变量", df.columns.tolist(), default=default_cont
        )
        ...
        with left:
            with st.form("ce1_form"):
                ...
                out_dir = st.text_input(
                    "输出目录", os.path.join(BASE_DIR, "streamlit_output")
                )
